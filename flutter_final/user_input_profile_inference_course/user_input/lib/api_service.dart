...
class ApiService {
    ...
    try {
      ...
      try {
        final healthCheck = await http.get(
          ...
          onTimeout: () {
            print('🔥 추천 서버 깨우기 타임아웃');
            throw Exception('Request timeout');
          },
        );
        print('🔥 추천 서버 상태 확인: ${healthCheck.statusCode}');
      } catch (e) {
        print('🔥 추천 서버 상태 확인 실패: $e');
      }
      
      // 2. 서버 준비 대기 시간 (Cold Start 고려)
      print('🔥 서버 준비 대기 중... (3초)');
      ...
      // 5. 실제 추천 API 호출 (타임아웃 설정)
      final response = await http.post(
        Uri.parse('$_recommendationBaseUrl/recommend/'),
        headers: {'Content-Type': 'application/json'},
        body: requestBody,
      ).timeout(
        const Duration(seconds: 30), // 타임아웃 설정
        onTimeout: () {
          print('🔥 추천 API 호출 타임아웃');
          throw Exception('Request timeout');
        },
      );
  ...
  static Future<Map<String, dynamic>?> getLiberalCourseRecommendations() async {
    try {
      ...
      // 1. 교양 추천 서버 깨우기 (타임아웃 설정)
      print('🔥 교양 추천 서버 깨우기 시도...');
      try {
        final healthCheck = await http.get(
      ...
      // 2. 서버 준비 대기 시간 (Cold Start 고려)
      print('🔥 교양 서버 준비 대기 중... (3초)');
      await Future.delayed(const Duration(seconds: 3));

      // 3. 실제 교양 추천 API 호출 (타임아웃 설정)
      final response = await http.post(
